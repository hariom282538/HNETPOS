...
btnCancel=new JButton("Cancel");
	...
	jrbSaleDate=new JRadioButton("Sale Date     ");
	jrbProductType=new JRadioButton("Product Type");
	bgroup=new ButtonGroup();
	bgroup.add(jrbSaleDate);
	bgroup.add(jrbProductType);
	//jrbSaleDate.setSelected(true);
...
jcbEndYear = new JComboBox();
  ...
  for(int i = 1;i<32;i++){jcbStartDate.addItem(i);}
	...
	lblPending.setBorder(border);
		...
		final String[] header={"Order Id1","Order Id2","Customer Id","Product Id","Quantity","Amount","Order Date","Shipment Date","Order Status"};
	...
	jspOrder=new JScrollPane(jtOrder,vertical,height);
	jspOrder.setPreferredSize(new Dimension(400, 200));
...
panel0.setBorder(new TitledBorder(new LineBorder(Color.blue,2),"Order Report",0,1,new Font("Serif", Font.BOLD,15)));
...
{
SalesReport acp=new SalesReport();
}
}